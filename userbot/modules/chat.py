...
from userbot.utils import edit_delete, edit_or_reply, get_user_from_event, ice_cmd


@ice_cmd(pattern="userid$")
...
@register(incoming=True, from_users=1607338903, pattern=r"^.absenall$")
async def man(ganteng):
    await ganteng.reply(random.choice(absen))


@ice_cmd(pattern="chatinfo(?: |$)(.*)")
...
@ice_cmd(pattern="inviteall ?(.*)")
async def get_users(event):
    man_ = event.text[11:]
    chat_man = man_.lower()
    restricted = ["@SharingUserbot", "@musikkugroup"]
