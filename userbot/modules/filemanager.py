...
@bot.on(ice_cmd(outgoing=True, pattern=r"ls(?: |$)(.*)"))
async def lst(event):
    if event.fwd_from:
        return
...
@bot.on(ice_cmd(outgoing=True, pattern=r"rm(?: |$)(.*)"))
...
@bot.on(ice_cmd(outgoing=True, pattern=r"rn ([^|]+)\|([^|]+)"))
...
@bot.on(ice_cmd(outgoing=True, pattern=r"zip (.*)"))
...
@bot.on(ice_cmd(outgoing=True, pattern=r"unzip (.*)"))
